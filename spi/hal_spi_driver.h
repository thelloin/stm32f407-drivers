...
typedef enum
{
	...
	HAL_SPI_STATE_BUSY_TX_RX = 0x32,  /*  Data TX and RX process is ongoing             */
...
typedef struct
{
	...
	uint32_t Direction;          /*  Specifies the SPI Derectional mode state. */
	...
	uint32_t NSS;                /*  Specifies whether the NSS signal is managed by
	                                 harware (NSS pin) or by software using the SSI bit. */
...
typedef struct __spi_handle_t
{
	...
	uint16_t               RxXferSize;    /* SPI Rx tranfer size */
...
* @param  *spi_handle : pointer to handle structure used to initialize the spi peripheral
* @retval None
*/
...
* @param  *spi_handle : pointer to handle structure
